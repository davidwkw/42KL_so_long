...
static void	calc_w_cam(int win_width, int win_height, t_cam *cam)
{
	...
	if (abs(w_y_offset) > win_height && (abs(w_y_offset) - win_height) <= cam->max_y_offset)
...
static void	display_handler(t_vars *mlx, struct timeval *tv)
{
	...
	calc_p_cam(&mlx->cam);
	mlx_put_image_to_window(mlx->mlx, mlx->win, p_img, mlx->cam.p_coords.x, mlx->cam.p_coords.y);
